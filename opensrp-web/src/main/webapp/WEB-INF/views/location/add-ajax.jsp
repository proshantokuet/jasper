...
<c:url var="listUrl" value="/location/location.html" />
				...
				<div class="col-md-12">
					...
					<div class="portlet box blue-madison">
						<div class="portlet-title">
							<div class="caption">
								<i class="fa fa-edit"></i><spring:message code="lbl.addLocation"/>
							</div>
								...
								<div class="form-group">
					              <div class="row">
					                <div class="col-lg-6 form-group"> 
					                  ...
					                  <div class="col-lg-6 form-group">
					                   <label class="control-label" for="code"> <spring:message code="lbl.code"/> <span class="required">* </span></label>
								...
								<div class="form-group">
					              <div class="row">
					                <div class="col-lg-6 form-group"> 
					                  ...
					                  <div class="col-lg-6 form-group">
					                   <label class="control-label" for="code"> <spring:message code="lbl.tag"/> <span class="required">* </span></label>
										<select class="form-control" id="locationTag" name="locationTag" required="required">
...
<jsp:include page="/WEB-INF/views/footer.jsp" />
 <script>
        ...
        function showHideOptions(nextDivId, tagOrLocationId, nextOptionId, tagOrLocation, tagId) {
            console.log(nextDivId, " ", tagOrLocationId, " ", nextOptionId, " ", tagOrLocation, " ", tagId);
            let selectedLocationTag = $("#locationTag").val();
            let parentLocationId = parseInt(tagOrLocationId.split("?")[0]);

            if (selectedLocationTag == '<%=LocationTags.COUNTRY.getId()%>') {
                parentLocationId = 0;
                $('#division').hide();
                $('#division-option').html("");
                $('#division-option').prop('required', false);
            }
            if (selectedLocationTag == '<%=LocationTags.DIVISION.getId()%>') {
                parentLocationId = '<%=LocationTags.BANGLADESH.getId()%>';
                $('#division').hide();
                $('#division-option').html("");
                $('#division-option').prop('required', false);
            }
        ...
        $('#locationTag').change(function (event) {
            let divisionTagId = '<%=LocationTags.DIVISION.getId()%>';

            $('#parentLocation').val(1);
            showHideOptions("division", divisionTagId, "division-option", "location-by-tag-id", '<%=LocationTags.DIVISION.getId()%>');
        ...
        $("#division-option").change(function(event) {
            let selectedDivision = $("#division-option").val();
            showHideOptions("district", selectedDivision, "district-option", "child-locations", '<%=LocationTags.DISTRICT.getId()%>');
        ...
        $("#district-option").change(function(event) {
            ...
            showHideOptions("upazila", selectedDistrict, "upazila-option", "child-locations", '<%=LocationTags.UPAZILA_CITY_CORPORATION.getId()%>');
        ...
        $("#upazila-option").change(function(event) {
            ...
            showHideOptions("pourasabha", selectedUpazila, "pourasabha-option", "child-locations", '<%=LocationTags.POURASABHA.getId()%>');
        ...
        $("#pourasabha-option").change(function(event) {
            ...
            showHideOptions("union", selectedPourasabha, "union-option", "child-locations", '<%=LocationTags.UNION_WARD.getId()%>');
        });
